...
export default async function AuthorsPage() {
  ...
  return <><SiteHeader/><main className="mx-auto max-w-6xl px-5 pb-28 pt-10 sm:px-8"><h1 className="text-4xl font-semibold">全部作者</h1><p className="mt-3 text-muted">按作者进入她们自己的作品合集。</p><div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{authors.map((author) => <CompactAuthorCard author={author} key={author.slug}/>)}</div></main><MobileNav/></>;
}
