...
import { createOwnCategory, deleteOwnCategory, moveOwnCategory } from "@/server/author-actions";
  ...
  return (
    <div className="max-w-2xl">
      ...
      <form action={createOwnCategory} className="mt-7 flex gap-3">
        ...
        <button className="primary-button whitespace-nowrap" type="submit">添加</button>
      </form>
      <ul className="mt-6 space-y-3">
        {categories.map((category, index) => (
          <li className="flex items-center gap-2 rounded-2xl border border-border bg-surface p-4" key={category.id}>
            <div className="min-w-0 flex-1"><strong>{category.name}</strong><p className="text-sm text-muted">{category._count.works} 件作品</p></div>
            ...
            <form action={deleteOwnCategory}><input type="hidden" name="categoryId" value={category.id} /><button className="text-sm text-red-600" type="submit">删除</button></form>
          </li>
        ))}
      </ul>
    </div>
