...
export default async function EditAuthorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="max-w-3xl"><h1 className="text-3xl font-semibold">编辑作者</h1><form action={updateManagedAuthor} className="mt-8 space-y-5"><input type="hidden" name="authorId" value={author.id} /><Input label="作者名称" name="name" value={author.name} /><label className="block text-sm font-medium">作者简介<textarea className="field-input mt-2 min-h-28 py-3" name="bio" defaultValue={author.bio} /></label>{author.accountUser ? <label className="block text-sm font-medium">登录微信 ID（仅后台可见）<input className="field-input mt-2 opacity-70" value={author.accountUser.wechatId} readOnly /></label> : null}<Input label="微信号（公开，访客可复制）" name="publicWechatId" value={author.publicWechatId} required={false} /><Input label="SEO 标题" name="seoTitle" value={author.seoTitle ?? ""} required={false} /><label className="block text-sm font-medium">SEO 描述<textarea className="field-input mt-2 min-h-24 py-3" name="seoDescription" defaultValue={author.seoDescription ?? ""} /></label><button className="primary-button" type="submit">保存</button></form><AuthorImages authorId={author.id} images={{ avatar: assetUrl(author.avatarAsset?.storageKey), cover: assetUrl(author.coverAsset?.storageKey), wechatQr: assetUrl(author.wechatQrAsset?.storageKey) }} />{user.role === UserRole.SUPER_ADMIN && author.accountUser ? <form action={setAuthorAccountStatus} className="mt-8"><input type="hidden" name="authorId" value={author.id} /><input type="hidden" name="status" value={author.accountUser.status === "ACTIVE" ? "SUSPENDED" : "ACTIVE"} /><button className="text-sm underline" type="submit">{author.accountUser.status === "ACTIVE" ? "停用作者账号" : "恢复作者账号"}</button></form> : null}</div>;
}
