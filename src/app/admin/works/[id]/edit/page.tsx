...
export default async function EditWorkPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ saved?: string }> }) {
  ...
  const work = await db.work.findUniqueOrThrow({ where: { id }, include: { author: { select: { hiddenWorkFields: true } }, mainAsset: true, environments: true, images: { include: { asset: true }, orderBy: [{ sortOrder: "asc" }, { createdAt: "asc" }] } } });
