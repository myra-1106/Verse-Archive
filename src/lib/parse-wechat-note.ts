...
const labels = ["作品名称", "名称", "支持环境", "环境", "直购价", "转发价", "功能说明", "功能", "转发要求", "购买须知"];
...
export function parseWechatNote(rawText: string): ParsedWechatNote {
  ...
  return {
    name: field(text, ["作品名称", "名称"]).split("\n")[0]?.trim() ?? "",
    directPriceYuan: price(field(text, ["直购价"])),
    repostPriceYuan: price(field(text, ["转发价"])),
    ...
    repostRequirements: field(text, ["转发要求"]),
    purchaseNotes: field(text, ["购买须知"]),
  };
}
